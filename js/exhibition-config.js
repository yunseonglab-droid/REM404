...
async function fetchLiveMetadata() {
  ...
  try {
    const response = await fetch(`https://firestore.googleapis.com/v1/${path}?key=${encodeURIComponent(apiKey)}`, { cache: "no-store", signal: controller.signal });
    if (!response.ok) throw new Error(`REM404_CONFIG_${response.status}`);
    const document = await response.json();
    const metadata = Object.fromEntries(Object.entries(document.fields || {}).map(([key, value]) => [key, decodeFirestoreValue(value)]));
  ...
  if (!loadingPromise) {
    loadingPromise = (async () => {
      try {
        ...
        if (applyMetadata(liveMetadata)) return liveMetadata;
      } catch (error) {
        console.warn("REM404 live config unavailable; using GitHub snapshot.", error);
      }
      applyMetadata(exhibitionSnapshot);
      return siteConfig.exhibition;
    })();
  }
...
export function getExhibitionPeriodText(language = "ko") {
  ...
  const format = (value) => new Intl.DateTimeFormat(locale, { year: "numeric", month: language === "ko" ? "long" : "short", day: "numeric" }).format(new Date(value));
  const period = startAt && endAt ? `${format(startAt)} – ${format(endAt)}` : startAt ? `${format(startAt)}${language === "ko" ? "부터" : " onward"}` : `${language === "ko" ? "종료 " : "Until "}${format(endAt)}`;
